...
let NowLoadingMapId = -1;
DataManager.loadMapData = function(mapId) {
    if (mapId > 0) {
        var filename = 'Map%1.json'.format(mapId.padZero(3));
        this._mapLoader = ResourceHandler.createLoader('data/' + filename, this.loadDataFile.bind(this, '$dataMap', filename));
        this.loadDataFile('$dataMap', filename);
        NowLoadingMapId = mapId;
    } else {
        this.makeEmptyMap();
    }
};
...
let MenuOpenMapName = "";
let _SWClearSaveId = 701;
DataManager.makeSavefileInfo = function() {
    var info = {};
    info.globalId   = this._globalId;
    info.title      = MenuOpenMapName;
    info.characters = $gameParty.charactersForSavefile();
    info.characters.push(["Clear" , 0]);

    if ($gameSwitches.value(_SWClearSaveId + 0)) info.characters.push(["ClearDoors" , 0]);
    if ($gameSwitches.value(_SWClearSaveId + 1)) info.characters.push(["ClearDoors" , 1]);
    if ($gameSwitches.value(_SWClearSaveId + 2)) info.characters.push(["ClearDoors" , 2]);
    if ($gameSwitches.value(_SWClearSaveId + 3)) info.characters.push(["ClearDoors" , 3]);
    if ($gameSwitches.value(_SWClearSaveId + 4)) info.characters.push(["ClearDoors" , 4]);
...
SceneManager.updateScene = function() {
    if (this._scene) {
        if (!this._sceneStarted && this._scene.isReady()) {
            ...
            MapMovedLightFlame = 30; //移動して10フレーム後にライト処理が実行され始める
        }
